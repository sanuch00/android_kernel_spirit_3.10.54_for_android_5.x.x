...
kal_bool is_factory_cable(void);
#endif

int	g_AtCmdChargeMode = 0;		//for at%charge
int	g_AtCmdUsbId = 0;     		//for at%usbiadc
int	g_AtCmdUsbAdc = 0;    		//for at%usbiadc
...
#ifdef CONFIG_LGE_CABLE_ID_DETECT
		/* If factory cable connected, do not check battery id */
		if (BMT_status.factory_cable)
			val->intval = 1;
#endif
		break;
...
/* LGE_CHANGE_S: [2014-09-24] aiden.song@lge.com */
/* Comment: function for AT command */
...
{
    return sprintf(buf, "%d\n", BMT_status.AtCmdChargingModeOff);
}
static ssize_t store_LGBM_AtCmdChargingModeOff(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    if (buf != NULL && size != 0)
    {
        if(buf[0] == '1')
        {
            BMT_status.AtCmdChargingModeOff = KAL_TRUE;
...
{
    int batExist = 0;
    batExist = BMT_status.charger_exist;
    return sprintf(buf, "%d\n", batExist);
}
...
#ifdef CONFIG_LGE_PM_SOC_SCALE
			if (BMT_status.UI_SOC > FG_SOC_MAX)
				BMT_status.UI_SOC = FG_SOC_MAX;
#else	/* MTK Original */
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (resetBatteryMeter == KAL_TRUE) {
		...
		if (bat_is_recharging_phase() == KAL_TRUE) {
			...
			if (BMT_status.SOC >= 100) {
				...
				/* force show battery level as full */
				BMT_status.UI_SOC = 100;
			} else {
				/* if battery is discharging, show real soc */
				mt_battery_Sync_UI_Percentage_to_Real();
			}
#else	/* MTK Original */
			BMT_status.UI_SOC = 100;
...
#ifdef CONFIG_LGE_PM_SOC_SCALE
	{
		...
		set_rtc_spare_fg_value(rtc_soc);
	}
#else	/* MTK Original */
...
static void wireless_update(struct wireless_data *wireless_data)
{
	...
	if (ac_data->WIRELESS_ONLINE != WIRELESS_ONLINE)
...
static void ac_update(struct ac_data *ac_data)
{
	...
	if (ac_data->AC_ONLINE != AC_ONLINE)
...
static void usb_update(struct usb_data *usb_data)
{
	...
	if (usb_data->USB_ONLINE != USB_ONLINE)
...
static kal_bool mt_battery_present_check(void)
{
	kal_bool battery_present = KAL_TRUE;
	int rc;

	rc = battery_charging_control(CHARGING_CMD_GET_BATTERY_STATUS, &battery_present);
	if (rc)
		return KAL_TRUE;

	if (battery_present == KAL_FALSE) {
		battery_xlog_printk(BAT_LOG_CRTI, "battery not exist!\n");
	}
...
void mt_battery_GetBatteryData(void)
{
	...
	BMT_status.voltage_now = bat_vol;
	BMT_status.current_now = get_bat_charging_current_level();
#endif
	...
	BMT_status.ZCV = ZCV;
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
	BMT_status.bat_exist = mt_battery_present_check();
...
static PMU_STATUS mt_battery_CheckBatteryTemp(void)
{
	...
	if (BMT_status.pseudo_batt_enabled) {
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Pseudo Battery Mode. bypass LCS.\n");
		return status;
	}
#endif
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
	...
	if (BMT_status.bat_exist == KAL_FALSE) {
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] battery not exist. use dummy temperature.\n");
	}
#endif
	...
	if (lcs_rsp.change_lvl != STS_CHE_NONE) {
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Charging State changed by battery temperature.\n");
	}
...
/* LGE_CHANGE_S: [2014-09-24] aiden.song@lge.com */
/* Comment: Assume that non-standard charger & factory cable is a standard USB. */
#ifdef CONFIG_LGE_CABLE_ID_DETECT
        BMT_status.factory_cable = is_factory_cable();
        if (CHR_Type_num == NONSTANDARD_CHARGER && BMT_status.factory_cable)
            BMT_status.charger_type = STANDARD_HOST;
#endif
/* LGE_CHANGE_E: [2014-09-24] aiden.song@lge.com */
...
#ifdef CONFIG_LGE_CABLE_ID_DETECT
        if (BMT_status.factory_cable)
            battery_xlog_printk(BAT_LOG_CRTI, "[BAT_thread]Factory Cable (%dk)\n", g_AtCmdUsbId);
#endif
	} else {
...
#ifdef CONFIG_LGE_CABLE_ID_DETECT
        BMT_status.factory_cable = KAL_FALSE;
#endif
#ifdef CONFIG_LGE_PM_AT_CMD_SUPPORT
		BMT_status.AtCmdChargingModeOff = KAL_FALSE;
#endif
#ifdef CONFIG_LGE_PM_CHARGING_SCENARIO
...
/* LGE_CHANGE_S: [2014-09-24] aiden.song@lge.com */
/* Comment: Detect Cable Type.*/
#ifdef CONFIG_LGE_CABLE_ID_DETECT
kal_bool is_factory_cable(void)
{
        kal_bool res = KAL_FALSE;
        usb_cable_type cableId = NO_INIT_CABLE;

        cableId = get_usb_type();
        if (cableId == LT_CABLE_56K || cableId == LT_CABLE_130K || cableId == LT_CABLE_910K) {
            if( cableId == LT_CABLE_56K)
                g_AtCmdUsbId = 56;          //for at%usbiadc
            else if(cableId == LT_CABLE_130K)
                g_AtCmdUsbId = 130;         //for at%usbiadc
            else if(cableId == LT_CABLE_910K)
                g_AtCmdUsbId = 910;         //for at%usbiadc

            battery_xlog_printk(BAT_LOG_CRTI, "[%s] factory cable detected. (%dk)\n", __func__, g_AtCmdUsbId);
            res = KAL_TRUE;
        } else {
            g_AtCmdUsbId = 0;                       //for at%usbiadc
            battery_xlog_printk(BAT_LOG_CRTI, "[%s] normal cable detected.\n", __func__);
            res = KAL_FALSE;
        }
...
#endif
#ifdef CONFIG_MTK_EXTERNAL_CHARGER_SUPPORT
	BMT_status.voltage_now = 0;
	BMT_status.current_now = 0;
#endif
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
	BMT_status.bat_exist = mt_battery_present_check();
#endif
...
#ifdef CONFIG_MTK_EXTERNAL_CHARGER_SUPPORT
	/* Do not need charger_hv_detect_sw_workaround_init().
	  External charger will detect OVP */
#else
...
#ifndef CONFIG_MTK_EXTERNAL_CHARGER_SUPPORT
