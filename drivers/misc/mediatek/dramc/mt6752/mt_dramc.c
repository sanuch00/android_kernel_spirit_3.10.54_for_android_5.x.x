...
/* LGE_CHANGE_S: [2014-12-09] sunbin.kang@lge.com */
/* Comment: indicate to dfs status for DS-5 streamline */
int indication_for_dfs_high_freq=1;
int get_dramc_dfs_status(void)
{
    return indication_for_dfs_high_freq;
}
EXPORT_SYMBOL(get_dramc_dfs_status);
/* LGE_CHANGE_E: [2014-12-09] sunbin.kang@lge.com */
...
void do_DRAM_DFS(int high_freq)
{
	...
	indication_for_dfs_high_freq=high_freq;
	...
	if (u4HWGatingEnable)
	{
		Reg_Sync_Writel((DRAMCAO_BASE_ADDR + 0x028), Reg_Readl((DRAMCAO_BASE_ADDR + 0x028)) & (~(0x01<<30)));     // cha DLLFRZ=0
		u4HWTrackR0 = Reg_Readl((DRAMCNAO_BASE_ADDR + 0x374));	// cha r0
		u4HWTrackR1 = Reg_Readl((DRAMCNAO_BASE_ADDR + 0x378));	// cha r1
		Reg_Sync_Writel((DRAMCAO_BASE_ADDR + 0x028), Reg_Readl((DRAMCAO_BASE_ADDR + 0x028)) |(0x01<<30));     	// cha DLLFRZ=1
	}
#endif
    //shuffer_done = 0;
	...
	if (high_freq == 1)
	{
		...
		// MR2 RL/WL set
              ...
              if( value ==0x44444444)
              ucDram_Register_Write(0x088, (ddr_type == TYPE_LPDDR3) ? 0x001C0002 : LPDDR2_MODE_REG_2);	//1866 AC timing
              else
	...
	else
	{
		...
		Reg_Sync_Writel((DDRPHY_BASE_ADDR + (0x186 << 2)), Reg_Readl(DDRPHY_BASE_ADDR + (0x186 << 2)) | 0x10000);	// Switch MEMPLL2 reset mode select
		Reg_Sync_Writel((DDRPHY_BASE_ADDR + (0x189 << 2)), Reg_Readl(DDRPHY_BASE_ADDR + (0x189 << 2)) | 0x10000);	// Switch MEMPLL3 reset mode select
