    ...
    if(K2HH)
    	{
		    if(enable == TRUE)
		    {
		        res = k2dh_SetBWRate(client, K2HH_ODR200);  // operating mode as 200HZ
		    }
		    else
		    {
...
static int k2hh_init_client(struct i2c_client *client)
{
   ...
   // buf[1] = 0x00;
    buf[1] = 0x20; // for CTRL_REG2 LPFc= 2Hz
	//buf[1] = 0x60; // for CTRL_REG2 LPFc=0.5Hz
    buf[2] = 0x00;
    buf[3] = 0x00;
    buf[4] = 0x00;
    buf[5] = 0x00;
   ...
   if(K2HH) //for K2HH
    {
    	...
    	res = k2dh_SetBWRate(client, K2HH_ODR200);
		if(res != K2DH_SUCCESS )
		{
		    return res;
		}
