...
void config_misc_info(int md_id, unsigned int base[], unsigned int size)
{
    ...
    if(NULL != base) {
		...
		else
		{
			...
			if (result == true)
			{
				//printk("jjm_debug : config_misc_info : Read LGE_FacReadNetworkCode_SBP Success!\n");
			}
			else
			{
...
void ccci_dump_md_register_region(int md_id, unsigned char* addr, unsigned char* mapaddr, unsigned int len)
{
#define CCCI_REDUCE_LOG
    ...
    for(index = 0; index <= len; index += 16)
    {
        CCCI_MSG_INF(md_id, "cci", "%03X: %08X %08X %08X %08X\n", index
                , *((unsigned int*) (mapaddr + index)) , *((unsigned int*) (mapaddr + index + 4))
                , *((unsigned int*) (mapaddr + index + 8)) , *((unsigned int*) (mapaddr + index + 12)));
    }
#else
    for(index = 0; index <= len; index += 4)
    {
        CCCI_MSG_INF(md_id, "cci", "[%08X]->[%08X]=%08x",(unsigned int)(addr+index)
                 , (unsigned int)(mapaddr+index) ,*((unsigned int*) (mapaddr+index)));
    }
#endif
}
...
void ccci_dump_md_register(int md_id)
{
    ...
    map_ptr=md1_mcu_ptr;
    CCCI_MSG_INF(md_id, "cci", "[%08X]->[%08X]=%08x", MD1_MCU_ADDR_BASE, (unsigned int)(map_ptr) ,*((unsigned int*) (map_ptr)));
    for(ptr=(unsigned char *)MD1_MCU_ADDR_BASE,idx=0x100;idx<=0x464;idx+=16)
    {
        // CCCI_MSG_INF(md_id, "cci", "[%08X]->[%08X]=%08x",(unsigned int)(ptr+idx), (unsigned int)(map_ptr+idx) ,*((unsigned int*) (map_ptr+idx)));
        CCCI_MSG_INF(md_id, "cci", "%03X: %08X %08X %08X %08X\n", idx
                , *((unsigned int*) (map_ptr + idx)) , *((unsigned int*) (map_ptr + idx + 4))
                , *((unsigned int*) (map_ptr + idx + 8)) , *((unsigned int*) (map_ptr + idx + 12)));
    }
