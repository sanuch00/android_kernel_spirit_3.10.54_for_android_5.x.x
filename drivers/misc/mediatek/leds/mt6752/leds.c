...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	switch(pmic_type){
		case MT65XX_LED_PMIC_NLED_ISINK0:
			...
			mt6325_upmu_set_isink_ch0_step(ISINK_1);//8mA
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	if (pmic_type == MT65XX_LED_PMIC_LCD_ISINK)
	{
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK0)
		{
			...
			/* LGE_CHANGE_S: [2014-09-02] yoonsik.chung@lge.com */
			/* Comment: Set duty to control brightness with level value*/
			if (level < 50) {
				mt6325_upmu_set_isink_dim0_duty(1);
			} else if (level < 100) {
				mt6325_upmu_set_isink_dim0_duty(10);
			} else if (level < 150) {
				mt6325_upmu_set_isink_dim0_duty(15);
			} else if (level < 150) {
				mt6325_upmu_set_isink_dim0_duty(20);
			} else {
				mt6325_upmu_set_isink_dim0_duty(30);
			}
			/* LGE_CHANGE_E: [2014-09-02] yoonsik.chung@lge.com */
			if (level) 
			{
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK1)
		{
			...
			/* Comment: Set duty to control brightness with level value*/
			if (level < 50) {
				mt6325_upmu_set_isink_dim1_duty(1);
			} else if (level < 100) {
				mt6325_upmu_set_isink_dim1_duty(10);
			} else if (level < 150) {
				mt6325_upmu_set_isink_dim1_duty(15);
			} else if (level < 150) {
				mt6325_upmu_set_isink_dim1_duty(20);
			} else {
				mt6325_upmu_set_isink_dim1_duty(30);
			}
			/* LGE_CHANGE_E: [2014-09-02] yoonsik.chung@lge.com */
			if (level) 
			{
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK3)
		{
			...
			/* Comment: Set duty to control brightness with level value*/
			if (level < 50) {
				mt6325_upmu_set_isink_dim3_duty(1);
			} else if (level < 100) {
				mt6325_upmu_set_isink_dim3_duty(10);
			} else if (level < 150) {
				mt6325_upmu_set_isink_dim3_duty(15);
			} else if (level < 150) {
				mt6325_upmu_set_isink_dim3_duty(20);
			} else {
				mt6325_upmu_set_isink_dim3_duty(30);
			}
			/* LGE_CHANGE_E: [2014-09-02] yoonsik.chung@lge.com */
			if (level) 
			{
...
/* LGE_CHANGE_S: [2014-10-31] jeongjoo.lee@lge.com */
/* Comment: Indicator Red LED porting in kernel*/
typedef struct {
    kal_uint32 isink0_step;
    kal_uint32 isink0_dim0_duty;
    kal_uint32 isink0_dim0_fsel;
    kal_uint32 isink0_breath0_tr1_sel;
    kal_uint32 isink0_breath0_tr2_sel;
    kal_uint32 isink0_breath0_tf1_sel;
    kal_uint32 isink0_breath0_tf2_sel;
    kal_uint32 isink0_breath0_ton_sel;
    kal_uint32 isink0_breath0_toff_sel;
} ISINK0_LED_Breathmode_set_table;

static ISINK0_LED_Breathmode_set_table led_scene_setting[] = {
	{0x01, 31, 999, 15, 15, 15, 15, 3, 0},// 0.charging
	{0x01, 31, 999,  7, 7, 7, 7, 3, 0},// 1.Power on
	{0x01, 31, 999,  7, 7, 7, 7, 3, 0},// 2.Power off
	{0x01, 31, 999,  0, 0, 0, 0, 0, 0},// 3.Alarm
	{0x01, 31, 999,  2, 2, 2, 2, 0, 0},// 4.Incomming call
	{0x01, 31, 999,  0, 0, 0, 0, 0, 0},// 5.Urgent Incomming call
	{0x01, 31, 999,  4, 4, 4, 4, 3, 2},// 6.In call
	{0x01, 31, 999,  0, 0, 0, 0, 4, 0},// 7.Knock_on / Knock_code_fail / Bluetooth connect / Bluetooth disconnect
	{0x01, 31, 999,  0, 0, 0, 0, 0, 4},// 8.Missed call / Urgent missed call
	{0x01, 31, 999,  7, 7, 7, 7, 3, 0},// 9.default
};
...
int mt_mt65xx_breath_mode(unsigned int patternid)
{
    ...
    led_scene_setting[patternid].isink0_step, led_scene_setting[patternid].isink0_dim0_duty, led_scene_setting[patternid].isink0_dim0_fsel, \
    led_scene_setting[patternid].isink0_breath0_tr1_sel, led_scene_setting[patternid].isink0_breath0_ton_sel, led_scene_setting[patternid].isink0_breath0_toff_sel);
    mt6325_upmu_set_rg_drv_isink0_ck_pdn(0);
    mt6325_upmu_set_rg_drv_isink0_ck_cksel(0);
    mt6325_upmu_set_isink_ch0_mode(PMIC_PWM_1);
    mt6325_upmu_set_isink_ch0_step(led_scene_setting[patternid].isink0_step);
    mt6325_upmu_set_isink_breath0_tr1_sel(led_scene_setting[patternid].isink0_breath0_tr1_sel);
    mt6325_upmu_set_isink_breath0_tr2_sel(led_scene_setting[patternid].isink0_breath0_tr2_sel);
    mt6325_upmu_set_isink_breath0_tf1_sel(led_scene_setting[patternid].isink0_breath0_tf1_sel);
    mt6325_upmu_set_isink_breath0_tf2_sel(led_scene_setting[patternid].isink0_breath0_tf2_sel);
    mt6325_upmu_set_isink_breath0_ton_sel(led_scene_setting[patternid].isink0_breath0_ton_sel);
    mt6325_upmu_set_isink_breath0_toff_sel(led_scene_setting[patternid].isink0_breath0_toff_sel);
    mt6325_upmu_set_isink_dim0_duty(led_scene_setting[patternid].isink0_dim0_duty);
    mt6325_upmu_set_isink_dim0_fsel(led_scene_setting[patternid].isink0_dim0_fsel);
    mt6325_upmu_set_isink_ch0_en(NLED_ON);
}
...
int mt_mt65xx_breath_mode_2(int freq, int duty, int trf, int ton,int toff)
{
    ...
    mt6325_upmu_set_isink_ch0_mode(PMIC_PWM_1);
    mt6325_upmu_set_isink_ch0_step(ISINK_1);//8mA
    mt6325_upmu_set_isink_breath0_tr1_sel(trf);
    mt6325_upmu_set_isink_breath0_tr2_sel(trf);
    mt6325_upmu_set_isink_breath0_tf1_sel(trf);
    mt6325_upmu_set_isink_breath0_tf2_sel(trf);
    mt6325_upmu_set_isink_breath0_ton_sel(ton);
    mt6325_upmu_set_isink_breath0_toff_sel(toff);
    mt6325_upmu_set_isink_dim0_duty(duty);
    mt6325_upmu_set_isink_dim0_fsel(freq);
    mt6325_upmu_set_isink_ch0_en(NLED_ON);
...
void mt_mt65xx_led_set_to_breathmode(void)
{
    ...
    mt6325_upmu_set_isink_ch0_mode(PMIC_PWM_1);
    mt6325_upmu_set_isink_ch0_step(ISINK_1);//8mA
    mt6325_upmu_set_isink_breath0_tr1_sel(0);
    mt6325_upmu_set_isink_breath0_tr2_sel(0);
    mt6325_upmu_set_isink_breath0_tf1_sel(0);
    mt6325_upmu_set_isink_breath0_tf2_sel(0);
    mt6325_upmu_set_isink_breath0_ton_sel(0);
    mt6325_upmu_set_isink_breath0_toff_sel(0);
    mt6325_upmu_set_isink_dim0_duty(31);
    mt6325_upmu_set_isink_dim0_fsel(999);
    //mt6325_upmu_set_isink_ch0_en(NLED_ON);
}
/* LGE_CHANGE_E: [2014-10-31] jeongjoo.lee@lge.com */
