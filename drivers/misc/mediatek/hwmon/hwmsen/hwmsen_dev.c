...
#if defined(CONFIG_LGE_AUTO_DETECT_ALSPS)
#include "../../alsps/alsps_device_check.h"
#endif

#define SENSOR_INVALID_VALUE -1
#define MAX_CHOOSE_G_NUM 5
#define MAX_CHOOSE_M_NUM 5
#define MAX_CHOOSE_P_NUM 2
...
struct sensor_init_info* alsps_init_list[MAX_CHOOSE_P_NUM]= {0}; //modified
#endif
...
//			if((idx == ID_LIGHT) ||(idx == ID_PRESSURE) 
			if((idx == ID_PRESSURE) 
			||(idx == ID_PROXIMITY) || (idx == ID_TEMPRERATURE) || (idx == ID_LIGHT))  
			{
			...
			{
				//LGE_CHANGE_S : 2012-11-28 taebum81.kim@lge.com always update data
				#if 0 // data changed, update the data
				if((sensor_data.values[0] != obj_data.sensors_data[idx].values[0])
					...
					|| (sensor_data.values[2] != obj_data.sensors_data[idx].values[2]))
				{
					...
					mutex_unlock(&obj_data.lock);
				#endif
				//LGE_CHANGE_E : 2012-11-28 taebum81.kim@lge.com always update data
			}
		}
	}
...
static ssize_t hwmsen_store_hwmdev(struct device *dev,
                                  struct device_attribute *attr, char *buf)
{
	return 0;
}
...
#if 1 /* block_warning_message */
static ssize_t hwmsen_store_sensordevnum(struct device *dev,
                                  struct device_attribute *attr, char *buf)
{
	return 0;
}
...
DEVICE_ATTR(hwmdev,     S_IWUSR | S_IRUGO, hwmsen_show_hwmdev, hwmsen_store_hwmdev);
...
#if 1 /* block_warning_message */
DEVICE_ATTR(hwmsensordevnum,      S_IWUSR | S_IRUGO, hwmsen_show_sensordevnum,  hwmsen_store_sensordevnum);
#else
