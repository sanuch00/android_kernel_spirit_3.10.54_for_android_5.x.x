...
int iReadDataFromBRCB032GWZ3(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
    ...
    CAM_CALDB("[BRCB032GWZ_3_eeprom] ui4_offset=%x ui4_offset(80)=%x ui4_offset(8)=%x\n",ui4_offset , (unsigned short)( (ui4_offset>>8) & 0x0000FFFF),SampleOffset);
    ...
    CAM_CALDB("[BRCB032GWZ_3_eeprom] EEPROM_Address[0]=%x EEPROM_Address[1]=%x\n",(EEPROM_Address[0]) ,(EEPROM_Address[1]));
    ...
    CAM_CALDB("[BRCB032GWZ_3_eeprom] loop[0]=%d loop[1]=%d\n",(loop[0]) ,(loop[1]));
    ...
    for(loopCount=0; loopCount < Read_NUMofEEPROM; loopCount++)
    {
        do 
        {
            ...
            {
                CAM_CALDB("[BRCB032GWZ_3_eeprom]1 loopCount=%d loop[loopCount]=%d puSendCmd[0]=%x puSendCmd[1]=%x, EEPROM(%x)\n",loopCount ,loop[loopCount],puSendCmd[0],puSendCmd[1],EEPROM_Address[loopCount] );
                ...
                if (i4RetValue != 0)
                {
                  CAM_CALDB("[BRCB032GWZ_3_eeprom] I2C iReadData failed!! \n");
                  return -1;
                }           
                u4IncOffset += 16;
                loop[loopCount] -= 16;
            ...
            {
                CAM_CALDB("[BRCB032GWZ_3_eeprom]2 loopCount=%d loop[loopCount]=%d puSendCmd[0]=%x puSendCmd[1]=%x \n",loopCount ,loop[loopCount],puSendCmd[0],puSendCmd[1] );
...
#endif //COMMON_CAM_CAL_DRV
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x000BCB32) && (EEPROM_Address[0]==0x0) )
            {
            ...
            else
            {
               ...
               {               	 
                   ...
                   //i4RetValue =  iReadDataFromM24C08F((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
                   i4RetValue = iReadDataFromBRCB032GWZ3(ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
                   #if 0
                   if(0<test_retry)
                   	{
...
EXPORT_SYMBOL(BRCB032GW_3_EEPROM_Ioctl);
#else
