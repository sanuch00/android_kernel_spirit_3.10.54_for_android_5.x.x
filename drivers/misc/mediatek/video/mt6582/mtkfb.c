...
//LGE_BSP_COMMON LGE_CHANGE_S 141031 naveen.v : AT%KCAL
#ifdef CONFIG_LGE_LUT_KCAL
#include <mach/fbcon.h>
#endif
//LGE_BSP_COMMON LGE_CHANGE_E 141031 naveen.v : AT%KCAL

#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 141030 jongwoo82.lee : AT%LCD */
#define LGE_UPDATE_SCREEN_IOCTL     108
bool lcd_test_mode = 0;
static void mtkfb_fb_8888_to_888(void* fb_va);
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141030 jongwoo82.lee : AT%LCD */
...
static struct fbcon_config mipi_fb_cfg =
{
	.width		= 0,
	.height		= 0,
	.stride		= 0, /* MTK recommend .stride  = (width + 31) & (~31), */
...
int mtkfb_display_kcal(int is_white)
{
	...
	if(is_white)
	{
                mipi_fb_cfg.width = DISP_GetScreenWidth();
                mipi_fb_cfg.height = DISP_GetScreenHeight();
		mipi_fb_cfg.stride = ALIGN_TO(mipi_fb_cfg.width, MTK_FB_ALIGNMENT);

                mipi_fb_cfg.base    = (void*) ioremap_nocache((unsigned int) fbdev->ovl_pa_base, mipi_fb_cfg.stride * mipi_fb_cfg.height * ((mipi_fb_cfg.bpp) / 8) * 3);        
                memset(mipi_fb_cfg.base, 0xffffffff, mipi_fb_cfg.stride * mipi_fb_cfg.height * ((mipi_fb_cfg.bpp) / 8) * 3);  // white initialize
                iounmap((void *)mipi_fb_cfg.base);
	}
//use  DISP_StartOverlayTransfer instead of  DISP_DirectOverlayTransfer start
	 if (DISP_IsDecoupleMode()) 
    {
        DISP_StartOverlayTransfer();
    } 
//use  DISP_StartOverlayTransfer instead of  DISP_DirectOverlayTransfer end
	sem_early_suspend_cnt++;
	up(&sem_early_suspend);
	return 0;
}
...
#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 141030 jongwoo82.lee : AT%LCD */
        lcd_test_mode = 0;
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141030 jongwoo82.lee : DISP_StartVsync */
        return 0;
...
////////////////////////////////////////////////
    ...
    case LGE_UPDATE_SCREEN_IOCTL:
    {
            ...
            fb_addr = (void*) ioremap_nocache((unsigned int) fbdev->fb_pa_base,  MTK_FB_XRESV * MTK_FB_YRES * (MTK_FB_BPP / 8) * 3);
            memcpy(fb_addr, fb_map, MTK_FB_XRESV * MTK_FB_YRES * (MTK_FB_BPP / 8) * 3);

            mtkfb_fb_8888_to_888((void*)fbdev->fb_va_base); 
            
            iounmap((void *)fb_addr);
    ...
    break;
#endif
/* LGE_BSP_COMMON LGE_CHANGE_E 141030 jongwoo82.lee : AT%LCD */
    default:
...
static void mtkfb_fb_8888_to_8888(struct fb_info *fb_info)
{
    ...
    unsigned int fbsize = x_virtual * yres * 4; // 4 for Bpp ARGB8888

    unsigned int *s = (unsigned int*) fb_info->screen_base;
    unsigned int *d = (unsigned int*) (fb_info->screen_base + fbsize);
    unsigned int src_argb8888 = 0;
    int j = 0;
    int k = 0;
    int wait_ret = 0;

    MTKFB_INFO("mtkfb_fb_8888_to_8888 xres=%d yres=%d fbsize=0x%X x_virtual=%d s=0x%08X d=0x%08X\n",
    ...
    d = (unsigned int *) (fb_info->screen_base + fbsize);
    memcpy(s, d, fbsize);
}
...
#endif
// copies from the original boot-up logo buffer to OVL-0 decouple buffer (and 8888 to 888 color convert)
static void mtkfb_fb_8888_to_888(void* fb_va)
{
    ...
    unsigned int *s = (unsigned int*)(fb_va);
    unsigned char *d = (unsigned char*) (fb_va + + DISP_GetFBRamSize() + DAL_GetLayerSize());
    unsigned int src_argb8888 = 0;
    int j = 0;
    int k = 0;

    MTKFB_INFO("8888_to_888, s = 0x%x, d=0x%x\n", s,d);
    for (j = 0; j < yres; ++j)
    {
        s = (unsigned int *)(fb_va + j * ALIGN_TO(xres, MTK_FB_ALIGNMENT) * 4); // 4 for Bpp of ARGB8888
        d = (char *)(fb_va + DISP_GetFBRamSize() + DAL_GetLayerSize() + j * xres * 3); // 3 for Bpp of RGB888
        for (k = 0; k < xres; ++k)
        {
            src_argb8888 = *s++;
            *d++ = ((src_argb8888 & 0xFF));
...
#if defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141224 jongwoo82.lee : change LCD FPS */
        lcd_fps = 6000;
#else    
...
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141224 jongwoo82.lee : change LCD FPS */
...
#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 140609 jongwoo82.lee : NULL check */
        if(res == NULL)
        {
            r = -1;
            goto cleanup;
        }
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 140609 jongwoo82.lee : NULL check */
...
#if 0  // sangcheol.seo@lge.com : PR-C 
...
/* sangcheol.seo@lge.com : [JDI in-cell LCD] need to send the sleep cmd during video mode*/
    DISP_PrepareSuspend();
    // Wait for disp finished.
    if (wait_event_interruptible_timeout(disp_done_wq, !disp_running, HZ/10) == 0)
    {
