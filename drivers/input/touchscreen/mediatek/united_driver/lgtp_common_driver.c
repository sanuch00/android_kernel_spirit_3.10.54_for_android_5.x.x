...
{
	struct lge_touch_data *pDriverData = container_of(to_delayed_work(work_init), struct lge_touch_data, work_init);
...
{
	struct lge_touch_data *pDriverData = container_of(to_delayed_work(work_irq), struct lge_touch_data, work_irq);
...
{
	struct lge_touch_data *pDriverData = container_of(to_delayed_work(work_upgrade), struct lge_touch_data, work_upgrade);
	...
	else
	{
		...
		if( strcmp(pDriverData->fw_info.fw_ic_version, pDriverData->fw_info.fw_bin_version) )
		#else
		if(pDriverData->fw_info.fw_type == OFFICIAL_VERSION	&& strcmp(pDriverData->fw_info.fw_ic_version, pDriverData->fw_info.fw_bin_version))
		#endif
		{
...
static ssize_t store_lpwg_notify(struct i2c_client *client, const char *buf, size_t count)
{
	...
	if( ( (E_TouchLpwgCmdType)type == T_LPWG_CMD_LCD_STATUS ) || ( (E_TouchLpwgCmdType)type == T_LPWG_CMD_PROXIMITY_STATUS ) )
	{
...
{
	struct lge_touch_data *ts = container_of(lge_touch_kobj, struct lge_touch_data, lge_touch_kobj);
...
static int sysfs_register(struct lge_touch_data *ts, struct attribute **attribute_list)
{
	...
	ret = kobject_init_and_add(&ts->lge_touch_kobj,	&lge_touch_kobj_type, ts->input_dev->dev.kobj.parent, "%s", LGE_TOUCH_NAME);
	if( ret < 0 )
	{
...
{
	struct lge_touch_data *ts =	container_of(h, struct lge_touch_data, early_suspend);
...
static int touch_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	struct lge_touch_data *pDriverData = container_of(self, struct lge_touch_data, fb_notif);
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#if defined ( CONFIG_MTK_TOUCHPANEL )
	/* register and enable */
	mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_TYPE, TouchIrqHandler, 1);
	TouchEnableIrq();
	#else
