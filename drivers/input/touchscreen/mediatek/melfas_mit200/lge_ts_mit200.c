...
static int mit_fw_version_check(struct mms_data *ts, struct touch_fw_info *info)
{
	char ver[2] = {0};
	u8 target[2] = {0};
	/* ticklewind.kim@lge.com_S 20150109 */
	...
	if (strcmp(ts->pdata->fw_product, bin_product_id) != 0) {
		TOUCH_INFO_MSG("F/W Product is not matched [Origin:%s][IC:%s][Bin:%s] \n", ts->pdata->fw_product, ts->module.product_code, bin_product_id);
		/* ticklewind.kim@lge.com_S 20150109 */
		if (strncmp(old_product_id, bin_product_id, 8) == 0) {
			TOUCH_ERR_MSG("there are possible that is old f/w for c/yk. Force F/W Upgrade Start\n");
			return 2;
		}
		/* ticklewind.kim@lge.com_E */
...
int mit_isc_page_read(struct mms_data *ts, u8 *rdata, int addr)
{
	...
	cmd[5] = (addr&0x00FF)>>0;
#ifdef USE_DMA
	return (i2c_msg_transfer(client, msg, ARRAY_SIZE(msg)) < 0);
#else
...
#else
		} while(mt_get_gpio_in(ts->pdata->int_pin) && (iterator < MAX_ITERATOR));
#endif
