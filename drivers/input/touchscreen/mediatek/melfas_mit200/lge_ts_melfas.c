...
static int mms_get_LCD_info(struct mms_data *ts, struct touch_fw_info *fw_info)
{
	...
	if (mms_i2c_read(client, MIT_LCD_SHOW_STAUS, &ts->dev.lcd_status, 1) < 0) {
		TOUCH_INFO_MSG("MIT_LCD_SHOW_STAUS read failed\n");
		return -EIO;
	}
	TOUCH_INFO_MSG("LCD Status : 0x%X\n", ts->dev.lcd_status);
...
static void mit_battery_thermal(struct mms_data *ts, char caller)
{
	...
	ret = (short)mit_atoi(data);
#else
	struct power_supply *power_supply = NULL;

	power_supply = power_supply_get_by_name("battery");
	if ( power_supply == NULL )
	{
		TOUCH_INFO_MSG("No Battery, chkeck battery \n");
		goto DATA_ERROR;
	}

	power_supply->get_property(power_supply, POWER_SUPPLY_PROP_TEMP, &ret);
#endif
	if (ret >= 2000)
		goto DATA_ERROR;
...
#else
			power_supply->get_property(power_supply, POWER_SUPPLY_PROP_PRESENT, &is_present);
#endif
			if (is_present == 0) {
				ret = 300;
...
static int set_tci_info(struct i2c_client *client)
{
	...
	ts->pdata->tci_info->sensitivity = 20; //for contacton thre on F/W
...
int mms_reset_power(struct i2c_client* client, int power_ctrl)
{
	struct mms_data* ts = get_touch_handle_(client);
	
	switch (power_ctrl) {
	case POWER_OFF:
		mt_set_gpio_out(ts_pdata->reset_pin, POWER_OFF);
		TOUCH_POWER_MSG("power: reset_pin low \n");
		break;
	case POWER_ON:
		mt_set_gpio_out(ts_pdata->reset_pin, POWER_ON);
...
static int lpwg_control(struct mms_data* ts, u8 mode)
{
	...
	TOUCH_INFO_MSG("lpwg_mode_old[%d]\n", ts->pdata->lpwg_mode_old);
	TOUCH_INFO_MSG("lpwg_mode[%d]\n", mode);
LPWG_NOT_USE:
	return 0;
}
...
static int mms_ic_ctrl(struct i2c_client *client, u32 code, u32 value)
{
	...
	case IC_CTRL_LPWG:
		...
		{
			TOUCH_INFO_MSG("LPWG mode be already setted [%d / %d]\n", (u8)param->v1, ts->pdata->lpwg_mode_old);
			return 0;
		}
		else
			TOUCH_INFO_MSG("value[%d], ts->pdata->lpwg_mode_old[%d]\n", (u8)param->v1, ts->pdata->lpwg_mode_old);
		...
		//ticklewind.kim_S:150101
		if ( lpwg_control(ts, (u8)param->v1) == -EIO )
		{
			TOUCH_INFO_MSG("lpwg_contorl fail, so retry to reset ic");
			mms_power_reset(ts);
			if ( lpwg_control(ts, (u8)param->v1) == -EIO ) 
				TOUCH_ERR_MSG("after reset, lpwg_control fail...again. check ic\n");
		}
		//ticklewind.kim_E:150101
