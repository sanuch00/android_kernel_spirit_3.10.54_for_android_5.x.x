...
{
#if 1 /* LGE_BSP_COMMON : branden.you@lge.com_20141108 : */
	int ret = 0;
	u8 reg[2] = {0};
	u8 data[2] = {0};
...
int Lu201x_InterruptHandler(struct i2c_client *client,TouchReadData *pData)
{
	...
	/* read touch data */
    if(Lu20xx_I2C_Read(client, FW_STATUS, (u8 *)&touch_data.FWStatus, 1) != 0 )
    {
...
static void Lu201x_NotifyHandler(struct i2c_client *client, TouchNotify notify, int data)
{
	switch( notify )
	{
		case NOTIFY_CALL:
...
TouchDeviceSpecificFunction Lu201x_Func = {
	...
	.NotifyHandler = Lu201x_NotifyHandler,
