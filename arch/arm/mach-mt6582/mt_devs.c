...
unsigned long g_bl_mem_sz = 0;
unsigned int get_total_DRAM_size(void);
#endif
...
static struct mt_chip_conf chip_config = {
    .setuptime = 3,
    .holdtime = 3,
    .high_time = 2,
	.low_time = 1,
...
#endif /* defined(CONFIG_LGE_BROADCAST_SBTVD_LATIN) */
#endif
...
/* Charge pump driver                                                    */
/*=======================================================================*/
/* LGE_CHANGE: sangcheol.seo@lge.com 20140925 : add backlight device as charge pump*/
#if defined (CONFIG_BACKLIGHT_LM3639) || defined (CONFIG_BACKLIGHT_LM3632) || defined (CONFIG_CUSTOM_KERNEL_CHARGEPUMP)
...
static struct platform_device rt9536_dev = {
	.name = "rt9536",
	.id   = -1,
};
#endif
#ifdef CONFIG_MTK_EXTERNAL_CHARGER_MAX8971
/* device will be added in max8971.c */
#endif
#endif /* CONFIG_MTK_EXTERNAL_CHARGER_SUPPORT */
...
"\n",   bl_fb.base, bl_fb.size, (unsigned long long)FB_START, (unsigned long long)FB_SIZE);
		...
		if ((bl_fb.base != (unsigned long long)FB_START) || (bl_fb.size != (unsigned long long)FB_SIZE))
		{
			...
			else
			{
				...
				{
					mtkfb_set_fb_lk((unsigned long long)bl_fb.base,(unsigned long long)bl_fb.size);
				}
			}
		}
...
static struct spi_board_info spi_board_devs[] __initdata = {
	[0] = {
		...
		.chip_select = 0,
		.mode = SPI_MODE_0,
		.max_speed_hz = 10*100*1000,//24MHz
},
};
#endif
...
//G3 Stylus and L80+
        ...
        break;
#endif
        retval = platform_device_register(&rt9536_dev);
        if (retval != 0) {
            return retval;
        }
    } while (0);
...
unsigned int get_total_DRAM_size(void)
{
	...
	#if defined (CONFIG_LGE_HANDLE_PANIC) || defined (CONFIG_LGE_HIDDEN_RESET)
	total_size = (g_bl_mem_sz + LGE_BSP_PRELOADER_MEM_SIZE + LGE_BSP_RESERVED_MEM_SIZE + LGE_BSP_LK_MEM_SIZE);
	#else
	total_size = g_bl_mem_sz;
	#endif

	printk("[ONE_BIN_MEMORY] : Kernel Total DRAM size = 0x%x", total_size);
